#pragma once
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "System/zzzz__Object_def.hpp"
#include "beatsaber-hook/shared/utils/typedefs-string.hpp"
CORDL_MODULE_EXPORT(ArgumentBase)
namespace System {
class Type;
}
// Forward declare root types
namespace GlobalNamespace {
class ArgumentBase;
}
// Write type traits
MARK_REF_PTR_T(::GlobalNamespace::ArgumentBase);
// Type: ::ArgumentBase
namespace GlobalNamespace {
// Is value type: false
// Dependencies: {TypeDefinitionIndex(TypeDefinitionIndex(2613))}
// Self: TypeDefinitionIndex(TypeDefinitionIndex(5978))
// CS Name: ::ArgumentBase*
class CORDL_TYPE ArgumentBase : public ::System::Object {
public:
// Declarations
/// @brief The size of the true reference type
static constexpr auto  __IL2CPP_REFERENCE_TYPE_SIZE{0x28};

/// @brief The size this ref type adds onto its base type, may evaluate to 0
 uint8_t  __fields[0x28 - sizeof(::System::Object)]{};

/// @brief Field name offset 0x10
 __declspec(property(get=__get_name, put=__set_name)) ::StringW  name;

/// @brief Field description offset 0x18
 __declspec(property(get=__get_description, put=__set_description)) ::StringW  description;

/// @brief Field valueType offset 0x20
 __declspec(property(get=__get_valueType, put=__set_valueType)) ::System::Type*  valueType;

 __declspec(property(get=get_isOptional)) bool  isOptional;

constexpr void __set_name(::StringW  value) ;

constexpr ::StringW& __get_name() ;

constexpr ::StringW const& __get_name() const;

constexpr void __set_description(::StringW  value) ;

constexpr ::StringW& __get_description() ;

constexpr ::StringW const& __get_description() const;

constexpr void __set_valueType(::System::Type*  value) ;

constexpr ::System::Type* __get_valueType() ;

constexpr ::cordl_internals::to_const_pointer<::System::Type*> __get_valueType() const;

/// @brief Method get_isOptional addr 0x0 size 0xffffffffffffffff virtual true final false
inline bool get_isOptional() ;

static inline ::GlobalNamespace::ArgumentBase* New_ctor(::StringW  name, ::StringW  description, ::System::Type*  valueType) ;

/// @brief Method .ctor addr 0x230dcc4 size 0x3c virtual false final false
inline void _ctor(::StringW  name, ::StringW  description, ::System::Type*  valueType) ;

/// @brief Method TryParseWithValue addr 0x0 size 0xffffffffffffffff virtual true final false
inline bool TryParseWithValue(::StringW  value) ;

/// @brief Method Reset addr 0x0 size 0xffffffffffffffff virtual true final false
inline void Reset() ;

// Ctor Parameters [CppParam { name: "", ty: "ArgumentBase", modifiers: "&&", def_value: None }]
// @brief delete move ctor to prevent accidental deref moves
ArgumentBase(ArgumentBase && ) = delete;

// Ctor Parameters [CppParam { name: "", ty: "ArgumentBase", modifiers: "const&", def_value: None }]
// @brief delete copy ctor to prevent accidental deref copies
ArgumentBase(ArgumentBase const& ) = delete;

protected:
// Ctor Parameters []
// @brief default ctor
 ArgumentBase()  = default;
public:


// Fields


// Properties


// Methods

static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(::cordl_internals::size_check_v<::GlobalNamespace::ArgumentBase, 0x28>, "Size mismatch!");

} // namespace end def GlobalNamespace
NEED_NO_BOX(::GlobalNamespace::ArgumentBase);
DEFINE_IL2CPP_ARG_TYPE(::GlobalNamespace::ArgumentBase*, "", "ArgumentBase");
