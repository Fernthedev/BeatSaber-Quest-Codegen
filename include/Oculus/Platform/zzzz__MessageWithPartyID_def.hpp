#pragma once
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "Oculus/Platform/Models/zzzz__PartyID_def.hpp"
#include "Oculus/Platform/zzzz__Message_1_def.hpp"
#include <cstdint>
CORDL_MODULE_EXPORT(MessageWithPartyID)
namespace Oculus::Platform::Models {
class PartyID;
}
// Forward declare root types
namespace Oculus::Platform {
class MessageWithPartyID;
}
// Write type traits
MARK_REF_PTR_T(::Oculus::Platform::MessageWithPartyID);
// Type: Oculus.Platform::MessageWithPartyID
namespace Oculus::Platform {
// Is value type: false
// Dependencies: {TypeDefinitionIndex(TypeDefinitionIndex(13262)), TypeDefinitionIndex(TypeDefinitionIndex(13467)), GenericInstantiation(GenericInstantiation { tdi: TypeDefinitionIndex(13262), inst: 4603 })}
// Self: TypeDefinitionIndex(TypeDefinitionIndex(13314))
// CS Name: ::Oculus.Platform::MessageWithPartyID*
class CORDL_TYPE MessageWithPartyID : public ::Oculus::Platform::Message_1<::Oculus::Platform::Models::PartyID*> {
public:
// Declarations
/// @brief The size of the true reference type
static constexpr auto  __IL2CPP_REFERENCE_TYPE_SIZE{0x30};

/// @brief The size this ref type adds onto its base type, may evaluate to 0
 uint8_t  __fields[0x30 - sizeof(::Oculus::Platform::Message_1<::Oculus::Platform::Models::PartyID*>)]{};

static inline ::Oculus::Platform::MessageWithPartyID* New_ctor(::cordl_internals::intptr_t  c_message) ;

/// @brief Method .ctor addr 0x26f3248 size 0x58 virtual false final false
inline void _ctor(::cordl_internals::intptr_t  c_message) ;

/// @brief Method GetPartyID addr 0x26f32a0 size 0x3c virtual true final false
inline ::Oculus::Platform::Models::PartyID* GetPartyID() ;

/// @brief Method GetDataFromMessage addr 0x26f32dc size 0x9c virtual true final false
inline ::Oculus::Platform::Models::PartyID* GetDataFromMessage(::cordl_internals::intptr_t  c_message) ;

// Ctor Parameters [CppParam { name: "", ty: "MessageWithPartyID", modifiers: "&&", def_value: None }]
// @brief delete move ctor to prevent accidental deref moves
MessageWithPartyID(MessageWithPartyID && ) = delete;

// Ctor Parameters [CppParam { name: "", ty: "MessageWithPartyID", modifiers: "const&", def_value: None }]
// @brief delete copy ctor to prevent accidental deref copies
MessageWithPartyID(MessageWithPartyID const& ) = delete;

protected:
// Ctor Parameters []
// @brief default ctor
 MessageWithPartyID()  = default;
public:


// Methods

static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(::cordl_internals::size_check_v<::Oculus::Platform::MessageWithPartyID, 0x30>, "Size mismatch!");

} // namespace end def Oculus::Platform
NEED_NO_BOX(::Oculus::Platform::MessageWithPartyID);
DEFINE_IL2CPP_ARG_TYPE(::Oculus::Platform::MessageWithPartyID*, "Oculus.Platform", "MessageWithPartyID");
