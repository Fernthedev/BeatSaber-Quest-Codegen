...
struct CORDL_TYPE AvatarSystemIdentifier : public ::bs_hook::ValueTypeWrapper<0x10> {
public:
// Declarations
...
/// @brief Field hash offset 0x8
 __declspec(property(get=__get_hash, put=__set_hash)) uint32_t  hash;
...
constexpr void __set_hash(uint32_t  value) ;

constexpr uint32_t& __get_hash() ;

constexpr uint32_t const& __get_hash() const;
...
constexpr explicit AvatarSystemIdentifier(std::array<std::byte, __IL2CPP_VALUE_TYPE_SIZE>  instance) noexcept : ::bs_hook::ValueTypeWrapper<0x10>(instance) {
}

// Ctor Parameters []
// @brief default ctor
 AvatarSystemIdentifier()  = default;
