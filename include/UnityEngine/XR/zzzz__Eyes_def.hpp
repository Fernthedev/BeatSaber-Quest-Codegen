...
struct CORDL_TYPE Eyes : public ::bs_hook::ValueTypeWrapper<0x10> {
public:
// Declarations
...
/// @brief Field m_FeatureIndex offset 0x8
 __declspec(property(get=__get_m_FeatureIndex, put=__set_m_FeatureIndex)) uint32_t  m_FeatureIndex;
...
constexpr uint32_t& __get_m_FeatureIndex() ;

constexpr uint32_t const& __get_m_FeatureIndex() const;
...
constexpr explicit Eyes(std::array<std::byte, __IL2CPP_VALUE_TYPE_SIZE>  instance) noexcept : ::bs_hook::ValueTypeWrapper<0x10>(instance) {
}
